...
export interface PosterDrumProps {
  items?: PosterItem[];
  radius?: number;
  itemWidth?: number;
  itemHeight?: number;
  /** Degrees per 60fps frame when idle. 0 disables drift. */
  idleSpeed?: number;
  /** Degrees rotated per pixel of pointer drag. */
  dragSensitivity?: number;
  enableInertia?: boolean;
  showHud?: boolean;
  /** Controlled active index. Disables idle drift and drag. */
  activeIndex?: number;
  onActiveChange?: (item: PosterItem, index: number) => void;
  renderItem?: (item: PosterItem, isActive: boolean, index: number) => ReactNode;
  className?: string;
}

const DEFAULT_ITEMS: PosterItem[] = [
...
function computeRadius(itemWidth: number, itemCount: number, requested: number): number {
...
export default function PosterDrum({
  ...
  const [active, setActive] = useState<number>(typeof activeIndex === 'number' ? activeIndex : 0);
  const isControlled = typeof activeIndex === 'number';

  useEffect(() => {
    if (isControlled && typeof activeIndex === 'number') {
