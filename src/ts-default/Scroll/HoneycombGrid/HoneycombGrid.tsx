...
export interface HoneycombGridProps {
  apps?: AppItem[];
  /** Cells in a "wide" row. Narrow rows have wideCount - 1. Default 4 → 4/3. */
  wideCount?: number;
  /** Fraction of viewport width the wide row should span. */
  fillFactor?: number;
  /** Gap between cells as a fraction of iconSize. */
  gapRatio?: number;
  /** 0–1. Higher = more dramatic edge shrink. */
...
export default function HoneycombGrid({
  ...
  // Cells are imperative DOM nodes so we can animate hundreds per frame
  // without React reconciliation. Pool is keyed by `${row}:${col}`.
