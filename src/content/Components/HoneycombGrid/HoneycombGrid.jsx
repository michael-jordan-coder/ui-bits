...
function appForCell(apps, row, col) {
...
function fisheyeTransform(cx, cy, viewport, strength) {
...
function useHoneycombScroll({ targetRef, onFrame, onTap, enabled }) {
...
export default function HoneycombGrid({
  ...
  const lastWideCountRef = useRef(-1);
  const lastIconSizeRef = useRef(-1);
  const lastAppsRef = useRef(apps);
  ...
  // Cells are imperative DOM nodes so we can animate hundreds of them per
  // frame without React reconciliation. Pool is keyed by `${row}:${col}`.
  const createCell = useCallback((row, col) => {
    const app = appForCell(appsRef.current, row, col);
    if (!app) return null;
    const size = iconSizeRef.current;
    const el = document.createElement('button');
    el.className = 'honeycomb-grid-cell';
    el.type = 'button';
  ...
  // The frame owns pointer capture while dragging, so per-cell pointerup never
  // fires. Hit-test on tap via elementFromPoint instead.
  ...
  const onFrame = useCallback(
    scrollY => {
      ...
      // iconSize is a float derived from clientWidth — compare with an epsilon.
  ...
  const rootClass = ['honeycomb-grid-frame', className].filter(Boolean).join(' ');

  return (
    <div ref={frameRef} className={rootClass} role="grid" aria-label="Honeycomb grid">
      <div ref={canvasRef} className="honeycomb-grid-canvas" />
      <div className="honeycomb-grid-vignette" aria-hidden="true" />
    </div>
  );
}
