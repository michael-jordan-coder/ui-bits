...
function coverTransform(d: number, rotation: number, spacing: number, depth: number): string {
...
function coverOpacity(d: number): number {
  const dist = Math.abs(d);
  if (dist <= 1) return 1;
...
export default function CoverFlow({
  ...
  const rootStyle: CSSProperties = {
    ...
    ...(accentColor ? ({ ['--cf-accent' as string]: accentColor } as CSSProperties) : null),
    ...style
  };
