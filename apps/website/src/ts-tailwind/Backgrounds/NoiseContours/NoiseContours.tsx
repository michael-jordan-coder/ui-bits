...
// utility classes. A stack of `levels` contour lines drifts across the surface,
// each line's y driven by a sum of three sine octaves of differing frequency,
// phase and drift speed for an organic, non-repetitive ridge. Single rAF loop,
// dpr-scaled canvas, ResizeObserver. Honors prefers-reduced-motion.
export interface NoiseContoursProps extends Omit<HTMLAttributes<HTMLDivElement>, 'children'> {
  color?: string;
  surfaceColor?: string;
  levels?: number;
  scale?: number;
...
export default function NoiseContours({
  ...
  useEffect(() => {
    ...
    const heightAt = (xNorm: number, lineIndex: number, t: number): number => {
      let sum = 0;
