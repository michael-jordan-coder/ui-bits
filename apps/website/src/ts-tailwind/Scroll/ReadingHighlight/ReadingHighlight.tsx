...
function Word({ index, total, progress, dim, bright, reduce, word }: WordProps) {
  ...
  return (
    <motion.span className="inline [will-change:color]" style={{ color: reduce ? bright : color }}>
      {word}{' '}
    </motion.span>
  );
}
...
export interface ReadingHighlightProps extends HTMLAttributes<HTMLDivElement> {
  text?: string;
  dim?: string;
  bright?: string;
  height?: number;
  ...
  return (
    ...
    >
      <div className="relative w-full" style={{ height: height * 2.4 }}>
        <div
          className="sticky top-0 box-border flex items-center justify-center overflow-hidden p-[clamp(28px,7%,56px)]"
          style={{ height }}
        >
          <p className="m-0 max-w-[38ch] text-[clamp(20px,3.4vw,30px)] font-semibold leading-[1.5] tracking-[-0.01em] [text-wrap:balance]">
            {words.map((word, i) => (
              <Word
                key={i}
                index={i}
