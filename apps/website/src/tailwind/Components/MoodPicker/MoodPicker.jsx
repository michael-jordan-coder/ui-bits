...
export default function MoodPicker({
  ...
  const [value, setValue] = useState(() => (moods.some(m => m.id === defaultValue) ? defaultValue : null));
  ...
  const blobTransition = prefersReduced ? { duration: 0 } : { type: 'spring', stiffness: 420, damping: 26 };
  ...
  return (
    <div
      className={join('inline-flex select-none flex-col items-center gap-4 text-white', className)}
      ref={scope}
      {...rest}
    >
      {showHero && (
        <div className="grid place-items-center" style={{ height: heroSize }}>
          <div
            ref={heroRef}
            className="[will-change:transform,border-radius]"
            aria-hidden="true"
            style={{
              width: heroSize,
              height: heroSize,
        ...
        {moods.map((m, i) => {
          ...
          return (
            <motion.button
              ...
              disabled={readOnly}
              className="flex-none cursor-pointer border-none bg-transparent p-0 outline-none [-webkit-tap-highlight-color:transparent] [will-change:transform,opacity] focus-visible:[outline:2px_solid_rgba(255,255,255,0.75)] focus-visible:[outline-offset:3px] disabled:cursor-default"
