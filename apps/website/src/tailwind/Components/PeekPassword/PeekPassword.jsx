...
export default function PeekPassword({
  ...
  const [pupil, setPupil] = useState({ x: 0, y: size * 0.02 });

  const passwordFocused = focusField === 'password';
  const pawY = passwordFocused ? (showPassword ? size * 0.16 : 0) : size * 0.5;

  const pawTransition = prefersReduced ? { duration: 0 } : { type: 'spring', stiffness: 320, damping: 30 };
  const pupilTransition = prefersReduced ? { duration: 0 } : { type: 'spring', stiffness: 500, damping: 30 };
