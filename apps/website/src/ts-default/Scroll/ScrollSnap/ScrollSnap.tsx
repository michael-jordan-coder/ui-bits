import { useRef, useState, useEffect, useCallback, type HTMLAttributes, type CSSProperties } from 'react';
...
function panelFill(accent: string, index: number) {
...
export default function ScrollSnap({ panels = 4, accent = '#3ecf8e', height = 460, className = '', ...rest }: ScrollSnapProps) {
        ...
        {Array.from({ length: total }, (_, i) => {
          ...
          return (
            <button
              ...
              style={{ '--scroll-snap-accent': accent } as CSSProperties}
              onClick={() => goTo(i)}
            />
          );
        })}
