...
// uniform comb.
function TickRing({ segments, accent }: TickRingProps) {
  ...
  return (
    <svg className="scroll-rotate-ring" viewBox={`0 0 ${VIEWBOX} ${VIEWBOX}`} aria-hidden="true">
      <circle className="scroll-rotate-track" cx={CENTER} cy={CENTER} r={RADIUS} />
      {ticks.map((t, i) => (
        <line
          key={i}
          x1={t.x1}
...
export default function ScrollRotate({
  ...
  return (
    <div {...rest} ref={ref} className={join('scroll-rotate', className)} style={{ height, ...rest.style }}>
      <div className="scroll-rotate-track-rail" style={{ height: height * 2.4 }}>
        <div className="scroll-rotate-stage" style={{ height }}>
          <motion.div
            className="scroll-rotate-dial"
            style={{
          ...
          >
            ...
            <motion.div
              className="scroll-rotate-readout"
              style={{ rotate: reduce ? `${-(turns * 360)}deg` : counterRotate }}
            >
              <motion.span className="scroll-rotate-value">{reduce ? '100%' : percent}</motion.span>
              <span className="scroll-rotate-label">scrolled</span>
            </motion.div>
          </motion.div>
        </div>
      </div>
