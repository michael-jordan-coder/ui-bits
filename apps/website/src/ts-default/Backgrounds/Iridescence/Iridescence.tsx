...
// Iridescence fragment shader. Shares LiquidChrome's machinery — a metaball
// scalar field, a pseudo-normal from its gradient, the reconstructed studio
// environment and ACES tone map — but coats the chrome in a thin-film sheen: a
// cosine "rainbow" palette is driven by the view angle and reflection elevation
// (with a slow time drift), so the metal shifts hue like oil on water as the
// blobs move. One blob tracks the pointer when interactive.
...
void main() {
  ...
  float phase = ndv * 2.4 + refl.y * 1.6 + uTime * 0.05 + 0.2;
  vec3 irid = iridPalette(phase);

  vec3 color = chrome * mix(vec3(1.0), irid, uIridescence);
  color += fresnel * irid * 1.3;

  vec3 halfKey = normalize(normalize(vec3(-0.45, 0.75, 0.55)) + viewDir);
  float keySpec = pow(saturate(dot(normal, halfKey)), 90.0);
  color += keySpec * vec3(1.0, 0.97, 0.90) * 1.5;
...
const compileShader = (gl: WebGLRenderingContext, type: number, src: string) => {
  ...
  if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
    console.error('Iridescence shader compile error:', gl.getShaderInfoLog(shader));
...
// A full-bleed WebGL background of iridescent metal: drifting metaballs shaded
// with the same reconstructed studio environment as LiquidChrome, then coated in
// a view-angle thin-film palette so the chrome shifts hue like oil on water. One
// blob tracks the pointer. Single rAF loop, dpr-scaled canvas, ResizeObserver.
...
export default function Iridescence({
  ...
  useEffect(() => {
    ...
    if (!gl) {
      console.warn('Iridescence: WebGL unavailable; showing static backdrop.');
      return undefined;
    }
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error('Iridescence program link error:', gl.getProgramInfoLog(program));
      return undefined;
    }
    ...
    const draw = (now: number) => {
      ...
      gl.uniform1f(uIridescence, iridescence);
  ...
  }, [speed, scale, exposure, roughness, iridescence, interactive, reduceMotion]);

  return (
    <div ref={wrapRef} className={`iridescence ${className}`.trim()} {...rest}>
