...
// A flat-top hexagon tessellation. Pulse waves travel outward from a slowly
// orbiting origin: each hex's brightness is a raised-cosine band of the wave
// phase (distance − time*speed), so concentric rings of cells light up and fade
// as each wave expands. Outlines are drawn faintly; lit cells fill with `color`
// at the computed alpha. Single rAF loop, dpr-scaled canvas, ResizeObserver.
...
export default function HexPulse({
  ...
  return (
    <div ref={wrapRef} className={`hex-pulse ${className}`.trim()} {...rest}>
      <canvas ref={canvasRef} className="hex-pulse-canvas" />
      {children != null && <div className="hex-pulse-content">{children}</div>}
    </div>
  );
}
