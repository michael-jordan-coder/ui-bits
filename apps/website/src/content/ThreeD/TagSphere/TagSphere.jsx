...
export default function TagSphere({
  ...
  // Accumulated rotation (yaw around Y, pitch around X) and angular velocity.
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (reducedRef.current) {
        // Static, readable layout: no spin, full opacity, no depth scaling.
        velYawRef.current = 0;
        velPitchRef.current = 0;
      } else if (draggingRef.current) {
        // velocity written by pointer handlers
      } else {
        // inertial decay toward the gentle idle drift
      ...
      // Clamp pitch so the sphere never flips fully over.
      const maxPitch = 1.2;
      ...
      for (let i = 0; i < N; i++) {
        ...
        // depth01: 0 = far back, 1 = front-facing.
  ...
  return (
    <div
      className={`tag-sphere${dragging ? ' is-dragging' : ''}${reduced ? ' is-reduced' : ''}${className ? ` ${className}` : ''}`}
      style={rootStyle}
    >
      <div
        ref={sceneRef}
        className="tag-sphere-scene"
        role="group"
        aria-label={ariaLabel}
        tabIndex={0}
        onKeyDown={onKeyDown}
      >
        <ul className="tag-sphere-cloud">
          {items.map((item, i) => {
