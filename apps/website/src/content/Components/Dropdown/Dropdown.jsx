...
const findFirstEnabled = options => options.findIndex(o => !o.disabled);
const findLastEnabled = options => {
...
export default function Dropdown({
  ...
  useEffect(() => {
    if (!open) return;
    const primed = selectedIndex >= 0 && !options[selectedIndex].disabled ? selectedIndex : findFirstEnabled(options);
    setActiveIndex(primed);
  }, [open, selectedIndex, options]);

  useEffect(() => {
    if (!open || activeIndex < 0 || !panelRef.current) return;
    const el = panelRef.current.querySelector(`[data-ui-dropdown-index="${activeIndex}"]`);
  ...
  return (
    ...
    >
      ...
      >
        <span className={join('ui-dropdown__value', !selectedOption && 'ui-dropdown__value--placeholder')}>
            ...
            return (
              ...
              >
                <div className="ui-dropdown__option-text">
                  ...
                  {opt.description && <span className="ui-dropdown__option-desc">{opt.description}</span>}
                </div>
                <Check size={14} strokeWidth={2.25} className="ui-dropdown__option-check" aria-hidden="true" />
              </div>
            );
          })}
        </div>
